      ...
      columnList: {
        ...
        language: {
          ...
          isIncluded: function ($state, model) {
            return "appointment" == model.getSubjectFromState();
          },
        },
        phone: {
          column: "phone.name",
      ...
      module.addExtraOperation("view", {
        ...
        operation: async function ($state, model) {
          await $state.go("appointment.calendar", {
            identifier: model.metadata.participantSite.getIdentifier(),
          });
        },
      });
    }
              ...
              cnRecordAddScope.save = async function () {
                ...
                var available = vacancy
                  ? vacancyAvailable(
                      ...
                      null,
                      vacancy.start,
                      cnRecordAddScope.record.duration,
                      cache
                    )
                  : false;
                    ...
                    if (1 < CnSession.role.tier || "operator+" == CnSession.role.name) {
                      ...
                      if (!datetime.isAfter(moment())) {
                        CnModalMessageFactory.instance({
                          ...
                          message:
                            "The time you have selected is in the past.  You can only create new " +
                  ...
                  vacancyModel.calendarModel.settings.eventClick = function (vacancy) {
                    ...
                    if (!datetime.isAfter(moment())) {
                      CnModalMessageFactory.instance({
                        title: "Invalid Appointment Time",
                        message:
                          "The vacancy you have selected is in the past.  You can only create new " +
                          "appointment using a vacancy in the future.",
                      }).show();
                    } else if (
                      vacancy.appointments >= vacancy.operators &&
                      ...
                      CnModalMessageFactory.instance({
                        title: "No Vacancy",
                        message:
                          "The start time you have selected does not have any vacancy.  You may only " +
        ...
        return {
          ...
          controller: function ($scope) {
                ...
                if ("duration" == property) {
                  ...
                  if (!available) {
                    if (
                      ...
                      var hours = $scope.model.viewModel.record.duration / 60;
                      var message =
                        "NOTE: The duration you have chosen will require the vacancy calendar to " +
                      ...
                      proceed = await CnModalConfirmFactory.instance({
                        title:
                          (available ? "Change" : "Overbook") + " Appointment",
                        message: message,
                      }).show();
                    }
                  }
            ...
            if ($scope.model.getEditEnabled()) {
              var listener = $scope.$watch(
                "model.viewModel.vacancyModel",
                function (vacancyModel) {
                  if (angular.isDefined(vacancyModel)) {
                    vacancyModel.calendarModel.settings.dayClick =
                      async function (date, event, view) {
                        ...
                        if (
                          ...
                          if (!datetime.isAfter(moment())) {
                            CnModalMessageFactory.instance({
                              title: "Invalid Appointment Time",
                              message:
                                "The time you have selected is in the past.  You can only change the " +
                            ...
                            var response = await CnModalConfirmFactory.instance(
                              {
                                title: "Overbook Appointment",
                                message:
                                  "NOTE: The time you have chosen will require the vacancy calendar to be " +
                            ...
                            if (response) {
                              ...
                              $scope.model.viewModel.record.start_vacancy_id =
                                null;
                    ...
                    vacancyModel.calendarModel.settings.eventClick =
                      async function (vacancy) {
                        ...
                        if (
                          ...
                          CnModalMessageFactory.instance({
                            title: "Invalid Appointment Time",
                            message:
                              "The vacancy you have selected is in the past.  You can only change the " +
                          ...
                          if (
                            ...
                            var message =
                              (available
                                ? ""
                                : "NOTE: The time you have chosen will require the " +
        ...
        var object = function (parentModel) {
          ...
          angular.extend(this, {
            onAdd: async function (record) {
              ...
              newRecord.getIdentifier = function () {
                return parentModel.getIdentifierFromRecord(newRecord);
              };
              ...
              if (!record.disable_mail) {
                await CnHttpFactory.instance({
                  path: "appointment/" + record.id + "?add_mail=1",
                }).patch();
              }
            },
            ...
            onNew: async function (record) {
              ...
              if (
                angular.isUndefined(parent.subject) &&
                "interview" == parentModel.getSubjectFromState()
              ) {
                parent = {
                  subject: "interview",
              ...
              if (angular.isDefined(parent.subject)) {
                ...
                parentModel.metadata.participantSite =
                  CnSession.siteList.findByProperty(
                    "id",
                    response.data.site_id
                  );

                // get the vacancy model linked to the participant's site
                this.vacancyModel = CnVacancyModelFactory.forSite(
                  parentModel.metadata.participantSite
                );
                ...
                parentModel.metadata.columnList.phone_id.enumList =
                  response.data.reduce((list, item) => {
                    list.push({
                      value: item.id,
                      name:
                        "(" + item.rank + ") " + item.type + ": " + item.number,
                    });
                    return list;
                  }, []);
        ...
        var object = function (parentModel, root) {
          ...
          angular.extend(this, {
            ...
            onPatch: async function (data) {
              ...
              var cnRecordCalendar =
                cenozo.getScopeByQuerySelector(".record-calendar");
      ...
      function (
        ...
        var object = function (site) {
          ...
          angular.extend(this, {
            ...
            viewModel: CnAppointmentViewFactory.instance(
              this,
            ...
            getAddEnabled: function () {
              ...
              return (
                !("appointment" == subject && "list" == action) &&
                "vacancy" != subject &&
            ...
            getDeleteEnabled: function () {
              return (
                ...
                "vacancy" != this.getSubjectFromState()
              );
            },
            ...
            getMetadata: async function () {
              ...
              for (var i = 1; i <= 8; i++) {
                ...
                this.metadata.columnList.duration.enumList.push({
                  ...
                  name:
                    (0 < hours
                      ? hours + " hour" + (1 < hours ? "s" : "")
                      : "") +
                    (0 < hours && 0 < minutes ? ", " : "") +
                    (0 < minutes
                      ? minutes + " minute" + (1 < minutes ? "s" : "")
                      : ""),
                });
              }
            },
