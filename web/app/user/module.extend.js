cenozoApp.extendModule({
  ...
  create: (module) => {
    ...
    function getEventFromAppointment(appointment, timezone) {
      var event = {};
      if (angular.isUndefined(appointment.subject))
      ...
      if (
        ...
        event = {
          ...
          getIdentifier: function () {
            return appointment.getIdentifier();
          },
        ...
        return {
          ...
          controller: function ($scope) {
            if (angular.isUndefined($scope.model)) {
              ...
              $scope.model.calendarModel = CnUserCalendarFactory.instance(
                $scope.model
              );
            }
        ...
        var object = function (parentModel) {
          ...
          this.settings.eventClick = async function (record) {
            ...
            if (angular.isUndefined(record.subject)) {
              console.warn(
                "Clicked on personal calendar event which is not an appointment."
              );
            } else {
              var appointmentModel = CnAppointmentModelFactory.forUser(
                parentModel.viewModel.record
              );
              if (appointmentModel.getViewEnabled()) {
                if (
                  "appointment" == record.subject &&
                  angular.isDefined(appointmentModule.actions.view)
                )
                  await $state.go(record.subject + ".view", {
          ...
          angular.extend(this, {
            ...
            onCalendar: async function (
              ...
              await appointmentModel.calendarModel.onCalendar(
                replace,
                minDate,
                maxDate,
                ignoreParent
          ...
          async function init(object) {
            ...
            parentModel.getViewEnabled = function () {
              return true;
            };
