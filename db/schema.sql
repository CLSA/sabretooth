...
-- Wed 07 Jun 2017 12:32:46 PM EDT
...
USE `sabretooth`$$
DROP TRIGGER IF EXISTS `sabretooth`.`appointment_has_vacancy_AFTER_UPDATE` $$
USE `sabretooth`$$
CREATE DEFINER = CURRENT_USER TRIGGER `sabretooth`.`appointment_has_vacancy_AFTER_UPDATE` AFTER UPDATE ON `appointment_has_vacancy` FOR EACH ROW
BEGIN
