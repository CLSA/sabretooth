...
class module extends \cenozo\service\participant\module
{
  ...
  public function prepare_read( $select, $modifier )
  {
    ...
    {
      // Joining to the temporary participant_max_queue table created below is computationally expensive
      // so it is only done when reading one participant's details only.
      if( !is_null( $this->get_resource() ) )
      {
        ...
        $modifier->left_join(
          sprintf( '( %s %s ) AS participant_max_queue', $join_sel->get_sql(), $join_mod->get_sql() ),
          'participant.id',
          'participant_max_queue.participant_id'
        );

        $join_mod = lib::create( 'database\modifier' );
        $join_mod->where( 'participant_max_queue.queue_id', '=', 'queue_has_participant.queue_id', false );
        $join_mod->where(
          'participant_max_queue.participant_id',
          '=',
